...
create or replace package body pkg_bftn is
  ...
  begin    
    insert into tmp_receipt@emidb
      select PERIOD, BANK, LOAN_TYPE,LOAN_ACC,LOAN_CAT,LOC_CODE,MEMO_NO,PAY_DATE,PAY_AMT,PURPOSE,ENTRY_USER,ENTRY_DATE,UPDATE_USER,UPDATE_DATE,LN_TYPE_BK,ERROR_CODE,RECEPT_SEQ.NEXTVAL ENT_SL_NO,PROCESSED,ORG_LOC_CODE,IDCP,LOAN_PRODUCT,ACTUAL_LOC_CODE, ACTUAL_ERROR, BRANCH_CODE
        from bftn_recipt r
       where r.prod_nature = 'EMI'
        and r.mig_date = p_date;
    ...
    insert into tmp_receipt@olddb
      select PERIOD, BANK, LOAN_TYPE,LOAN_ACC,LOAN_CAT,LOC_CODE,MEMO_NO,PAY_DATE,PAY_AMT,PURPOSE,ENTRY_USER,ENTRY_DATE,UPDATE_USER,UPDATE_DATE,LN_TYPE_BK,ERROR_CODE,RECEPT_SEQ.NEXTVAL ENT_SL_NO,PROCESSED,ORG_LOC_CODE,IDCP,ACTUAL_LOC_CODE, ACTUAL_ERROR, BRANCH_CODE
        from bftn_recipt r
       where r.prod_nature = 'OLD'
        and r.mig_date = p_date;
                 /*
      select r.prod_nature ,count(*) from bftn_recipt r group by prod_nature
      select PERIOD, BANK, LOAN_TYPE,LOAN_ACC,LOAN_CAT,LOC_CODE,MEMO_NO,PAY_DATE,PAY_AMT,PURPOSE,ENTRY_USER,ENTRY_DATE,UPDATE_USER,UPDATE_DATE,LN_TYPE_BK,ERROR_CODE,RECEPT_SEQ.NEXTVAL ENT_SL_NO,PROCESSED,ORG_LOC_CODE,IDCP,LOAN_PRODUCT,ACTUAL_LOC_CODE, ACTUAL_ERROR, BRANCH_CODE
        from bftn_recipt r
       where r.prod_nature = 'OCR';
      ...
      select PERIOD, BANK, LOAN_TYPE,LOAN_ACC,LOAN_CAT,LOC_CODE,MEMO_NO,PAY_DATE,PAY_AMT,PURPOSE,ENTRY_USER,ENTRY_DATE,UPDATE_USER,UPDATE_DATE,LN_TYPE_BK,ERROR_CODE,RECEPT_SEQ.NEXTVAL ENT_SL_NO,PROCESSED,ORG_LOC_CODE,IDCP,ACTUAL_LOC_CODE, ACTUAL_ERROR, BRANCH_CODE
        from bftn_recipt r
       where r.prod_nature = 'OLD'      
      select *from tmp_receipt@emidb d where ENTRY_USER	='BEFTN' and period like '%-21-22'	;
*/                     
  end sp_data_posting;
begin
  -- Initialization
