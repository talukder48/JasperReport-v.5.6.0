...
begin
 for idx in(select distinct substr(m.loan_code,13,1) product, substr(m.loan_code,1,1) type  from bhbfcprj.loan_mas m where m.loc_code=p_office)loop          
  bhbfcprj.lm_yr_end_posting.data_posting(p_fin_year,p_office,idx.type,idx.product,'0000000000000','99999999999999');
end loop;                    
end sp_data_posting;
...
procedure FinalClosing_Final (p_office in varchar2,p_fin_year in varchar2,p_period in varchar2,p_date1 in date,p_date2 in date)is
 ...
 begin
       ...
       bhbfcprj.generate_actual_loc_code_new(p_period);  
       /*memo validation process for a branch for all types of products */
       for idx in(select distinct substr(m.loan_code,13,1) product, substr(m.loan_code,1,1) type  from bhbfcprj.loan_mas m where m.loc_code=p_office)loop      
           bhbfcprj.Lm_Memo_Validation_Process.data_validation(p_office,p_period,'','',idx.product,idx.type,'','','','AP');
       ...
       for idx in(select distinct substr(m.loan_code,13,1) product, substr(m.loan_code,1,1) type  from bhbfcprj.loan_mas m where m.loc_code=p_office)loop           
                delete from bhbfcprj.beg_of_month where loc_code=p_office and substr(loan_code,1,1)=idx.type and substr(loan_code,13,1)=idx.product;
                delete from bhbfcprj.tmp_rpt_bal where  loc_code=p_office and substr(loan_code,1,1)=idx.type and substr(loan_code,13,1)=idx.product;
                delete from bhbfcprj.rpt_to_date where  loc_code=p_office and substr(loan_code,1,1)=idx.type and substr(loan_code,13,1)=idx.product;
       end loop;  
       commit;
       /*           Run Beg of Month process for a Branch                 */
       bhbfcprj.lm_month_beg_process.Month_Beg_proc_Start(p_office,'','','','',p_period);
        DBMS_OUTPUT.ENABLE (buffer_size => NULL);
       commit;
       
       /*          Account*Month checking after beg of month process      */
       select count(*)*12 into v_ac_total_month FROM bhbfcprj.LOAN_MAS  WHERE LOAN_MAS.loan_active='Y' and loc_code=p_office ;       
       select count(*) into v_beg_month_row FROM bhbfcprj.beg_of_month  WHERE   loc_code=p_office ;
      ...
      -- end if;
       ...
       for idx in(select distinct substr(m.loan_code,13,1) product, substr(m.loan_code,1,1) type  from bhbfcprj.loan_mas m where m.loc_code=p_office)loop          
               bhbfcprj.yr_end_calculation_process.yr_end_select_data(p_fin_year,p_office,idx.product,idx.type,'0000000000000','99999999999999');
       end loop; 
       commit;
        /*         Run Year End Classification Process for a branch          */
       for idx in(select distinct substr(m.loan_code,13,1) product, substr(m.loan_code,1,1) type  from bhbfcprj.loan_mas m where m.loc_code=p_office)loop          
               bhbfcprj.yr_end_ln_classification.Ln_Class_Select_Data(p_fin_year,p_office,idx.product,idx.type,'0000000000000','99999999999999');
               DBMS_OUTPUT.ENABLE (buffer_size => NULL);
       end loop; 
       commit;       
       /*         check exist for all account in loan-ledger statement        */
      select count(*) into V_acc from (
       SELECT LOAN_MAS.loan_code FROM bhbfcprj.LOAN_MAS  WHERE LOAN_MAS.loan_active='Y' and loc_code=p_office 
            minus  
      select distinct b.loan_code from bhbfcprj.tmp_rpt_bal  b where b.loc_code=p_office);
...
procedure Closing_half_yr (p_office in varchar2,p_fin_year in varchar2,p_period in varchar2,p_date1 in date,p_date2 in date)is
 ...
 begin
       ...
       /*          Run Year End Calculation Process for a branch            */
       for idx in(select distinct substr(m.loan_code,13,1) product, substr(m.loan_code,1,1) type  from bhbfcprj.loan_mas m where m.loc_code=p_office)loop          
               bhbfcprj.yr_end_calculation_proc_half.yr_end_select_data(p_fin_year,p_office,idx.product,idx.type,'0000000000000','99999999999999');
       end loop; 
       commit;
        /*         Run Year End Classification Process for a branch          */
       for idx in(select distinct substr(m.loan_code,13,1) product, substr(m.loan_code,1,1) type  from bhbfcprj.loan_mas m where m.loc_code=p_office)loop          
               bhbfcprj.half_yr_classification.Ln_Class_Select_Data(p_fin_year,p_office,idx.product,idx.type,'0000000000000','99999999999999');
               DBMS_OUTPUT.ENABLE (buffer_size => NULL);
       end loop; 
       commit;   
...
procedure FinalClosing (p_office in varchar2,p_fin_year in varchar2,p_period in varchar2,p_date1 in date,p_date2 in date)is
 ...
 begin
       ...
       select count(*) into v_acc_drop_exist from bhbfcprj.acc_drop where fin_year = p_fin_year and loc_code = p_office and (cum_prin_bal <> 0 or cum_int_bal <> 0);
         if v_acc_drop_exist>0 then
           raise v_account_drop_exception ;
         end if;
       /*             Error Finding in memo Data for a Branch          */  
      Select count(*) into V_error_exist_memo from bhbfcprj.TMP_RECEIPT
       ...
       select count(*) into V_check_memo_exist from(                                        
                   select   bank ,loan_type,loan_acc,loc_code,memo_no,pay_date,pay_amt,purpose  from bhbfcprj.tmp_receipt m where m.loc_code=p_office and m.purpose='M'
                                                         minus                                       
                   select   bank,substr(loan_code,1 ,1)  loan_type, substr(loan_code,2 ,8)  loan_acc,loc_code,memo_no,pay_date,pay_amt,purpose  from bhbfcprj.receipt m where m.loc_code=p_office and m.purpose='M'
         );
     ...
     --  end if;
       /*                    check BF data for the branch               */
        select count(*) into v_bf_exist from bhbfcprj.bf_correction  b where  b.loc_code=p_office and fin_year=p_fin_year;

       if v_bf_exist>0 then     
       /*              check duplicate BF data for the branch            */                   
          select count(*) into v_duplicate_bf_count from (select  loan_acc, count(*) from bhbfcprj.bf_correction where fin_year=p_fin_year and loc_code=p_office
       ...
       /*              Run BF Correction Process for a branch            */
              for idx in(select distinct substr(m.loan_code,13,1) product, substr(m.loan_code,1,1) type  from bhbfcprj.loan_mas m where m.loc_code=p_office)loop      
                  bhbfcprj.bf_correction_process.BF_CORR_Select_date(p_fin_year,p_office,idx.type,idx.product);
              end loop; 
           commit;   
          end if;  
      end if;    
...
begin
    ...
    bhbfcprj.generate_actual_loc_code_new(p_period);
  /*memo validation process for a branch for all types of products */
    bhbfcprj.Lm_Memo_Validation_Process.data_validation(p_office,
                                                             p_period,
                                                             '',
                                                             '',
                                                             p_product,
  ...
  /*  delete beg_of_month, tmp_rpt_bal,rpt_to_date for pre-process  */
    ...
    delete from bhbfcprj.beg_of_month
     where loc_code = p_office
       and substr(loan_code, 1, 1) = P_type
       and substr(loan_code, 13, 1) = p_product;
       
    delete from bhbfcprj.tmp_rpt_bal
     where loc_code = p_office
       and substr(loan_code, 1, 1) = P_type
       and substr(loan_code, 13, 1) = p_product;
    delete from bhbfcprj.rpt_to_date
     where loc_code = p_office
       and substr(loan_code, 1, 1) = P_type
       and substr(loan_code, 13, 1) = p_product;
  ...
  commit;
  /*           Run Beg of Month process for a Branch                 */
  bhbfcprj.lm_month_beg_process.Month_Beg_proc_Start(p_office,
                                                          '',
                                                          '',
                                                          p_product,
                                                          P_type,
  ...
  /*          Run Year End Calculation Process for a branch            */
    ...
    bhbfcprj.yr_end_calculation_process.yr_end_select_data(p_fin_year,
                                                                p_office,
                                                                p_product,
                                                                P_type,
                                                                '0000000000000',
  ...
  /*         Run Year End Classification Process for a branch          */
    ...
    bhbfcprj.yr_end_ln_classification.Ln_Class_Select_Data(p_fin_year,
                                                                p_office,
                                                                p_product,
                                                                P_type,
                                                                '0000000000000',
