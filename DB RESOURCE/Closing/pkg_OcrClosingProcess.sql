create or replace package OcrClosingProcess is
...
/*
begin
  OcrClosingProcess.FinalClosing('5005','2019-2020','JUN-19-20','01-JUL-2019','30-JUN-2020');
end;
*/
end OcrClosingProcess;
/
create or replace package body OcrClosingProcess is
procedure sp_data_posting(p_fin_year varchar2, p_office    varchar2) is
begin
 for idx in(select distinct substr(m.loan_code,13,1) product, substr(m.loan_code,1,1) type  from bhbfcocr.loan_mas m where m.loc_code=p_office)loop          
  bhbfcocr.lm_yr_end_posting.data_posting(p_fin_year,p_office,idx.type,idx.product,'','');
end loop;                    
end sp_data_posting;
...
procedure FinalClosing_Final (p_office in varchar2,p_fin_year in varchar2,p_period in varchar2,p_date1 in date,p_date2 in date)is
 ...
 begin
       ...
       bhbfcocr.generate_actual_loc_code_new(p_period);  
       /*memo validation process for a branch for all types of products */
       for idx in(select distinct substr(m.loan_code,13,1) product, substr(m.loan_code,1,1) type  from bhbfcocr.loan_mas m where m.loc_code=p_office)loop      
           bhbfcocr.Lm_Memo_Validation_Process.data_validation(p_office,p_period,'','',idx.product,idx.type,'','','','AP');
       ...
       for idx in(select distinct substr(m.loan_code,13,1) product, substr(m.loan_code,1,1) type  from bhbfcocr.loan_mas m where m.loc_code=p_office)loop           
                delete from bhbfcocr.beg_of_month where loc_code=p_office and substr(loan_code,1,1)=idx.type and substr(loan_code,13,1)=idx.product;
                delete from bhbfcocr.tmp_rpt_bal where  loc_code=p_office and substr(loan_code,1,1)=idx.type and substr(loan_code,13,1)=idx.product;
                delete from bhbfcocr.rpt_to_date where  loc_code=p_office and substr(loan_code,1,1)=idx.type and substr(loan_code,13,1)=idx.product;
       end loop;  
       commit;
       /*           Run Beg of Month process for a Branch                 */
       bhbfcocr.lm_month_beg_process.Month_Beg_proc_Start(p_office,'','','','',p_period);
        DBMS_OUTPUT.ENABLE (buffer_size => NULL);
       commit;
       
       /*          Account*Month checking after beg of month process      */
       select count(*)*12 into v_ac_total_month FROM bhbfcocr.LOAN_MAS  WHERE LOAN_MAS.loan_active='Y' and loc_code=p_office ;       
       select count(*) into v_beg_month_row FROM bhbfcocr.beg_of_month  WHERE   loc_code=p_office ;
    ...
    --   if v_ac_total_month<>v_beg_month_row then
      --     raise v_beg_of_month_Exception;
       ...
       for idx in(select distinct substr(m.loan_code,13,1) product, substr(m.loan_code,1,1) type  from bhbfcocr.loan_mas m where m.loc_code=p_office)loop          
               bhbfcocr.yr_end_calculation_process.yr_end_select_data(p_fin_year,p_office,idx.product,idx.type,'0000000000000','99999999999999');
       end loop; 
       commit;
        /*         Run Year End Classification Process for a branch          */
       for idx in(select distinct substr(m.loan_code,13,1) product, substr(m.loan_code,1,1) type  from bhbfcocr.loan_mas m where m.loc_code=p_office)loop          
               bhbfcocr.yr_end_ln_classification.Ln_Class_Select_Data(p_fin_year,p_office,idx.product,idx.type,'0000000000000','99999999999999');
               DBMS_OUTPUT.ENABLE (buffer_size => NULL);
       end loop; 
       commit;       
       /*         check exist for all account in loan-ledger statement        */
      select count(*) into V_acc from (
       SELECT LOAN_MAS.loan_code FROM bhbfcocr.LOAN_MAS  WHERE LOAN_MAS.loan_active='Y' and loc_code=p_office 
            minus  
      select distinct b.loan_code from bhbfcocr.tmp_rpt_bal  b where b.loc_code=p_office);
...
procedure FinalClosing (p_office in varchar2,p_fin_year in varchar2,p_period in varchar2,p_date1 in date,p_date2 in date)is
 ...
 begin
       ...
       select count(*) into v_acc_drop_exist from bhbfcocr.acc_drop where fin_year = p_fin_year and loc_code = p_office and (cum_prin_bal <> 0 or cum_int_bal <> 0);
         if v_acc_drop_exist>0 then
           raise v_account_drop_exception ;
         end if;
       /*             Error Finding in memo Data for a Branch          */  
      Select count(*) into V_error_exist_memo from bhbfcocr.TMP_RECEIPT
       ...
       select count(*) into V_check_memo_exist from(                                        
                   select   bank ,loan_type,loan_acc,loc_code,memo_no,pay_date,pay_amt,purpose  from bhbfcocr.tmp_receipt m where m.loc_code=p_office and m.purpose='M'
                                                         minus                                       
                   select   bank,substr(loan_code,1 ,1)  loan_type, substr(loan_code,2 ,8)  loan_acc,loc_code,memo_no,pay_date,pay_amt,purpose  from bhbfcocr.receipt m where m.loc_code=p_office and m.purpose='M'
         );
       ...
       /*                    check BF data for the branch               */
        select count(*) into v_bf_exist from bhbfcocr.bf_correction  b where  b.loc_code=p_office and fin_year=p_fin_year;

       if v_bf_exist>0 then     
       /*              check duplicate BF data for the branch            */                   
          select count(*) into v_duplicate_bf_count from (select  loan_acc, count(*) from bhbfcocr.bf_correction where fin_year=p_fin_year and loc_code=p_office
       ...
       /*              Run BF Correction Process for a branch            */
              for idx in(select distinct substr(m.loan_code,13,1) product, substr(m.loan_code,1,1) type  from bhbfcocr.loan_mas m where m.loc_code=p_office)loop      
                  bhbfcocr.bf_correction_process.BF_CORR_Select_date(p_fin_year,p_office,idx.type,idx.product);
              end loop; 
           commit;   
          end if;  
      end if;    
  ...
  exception 
    ...
    when v_loan_ledger_exception then
        dbms_output.put_line('All account is not posting for -'||p_office);   
    when v_beg_of_month_Exception then   
        dbms_output.put_line('Beg of month is not run for  -'||p_office); 
    when v_memo_exist_exception then 
        dbms_output.put_line('All memo is not posting to recipt from tmp_recipt -'||p_office); 
    when v_account_drop_exception then 
        dbms_output.put_line('Account Drop exist, Please run account drop process -'||p_office);   
...
begin
    ...
    bhbfcocr.generate_actual_loc_code_new(p_period);
  /*memo validation process for a branch for all types of products */
    bhbfcocr.Lm_Memo_Validation_Process.data_validation(p_office,
                                                             p_period,
                                                             '',
                                                             '',
                                                             p_product,
  ...
  /*  delete beg_of_month, tmp_rpt_bal,rpt_to_date for pre-process  */
    ...
    delete from bhbfcocr.beg_of_month
     where loc_code = p_office
       and substr(loan_code, 1, 1) = P_type
       and substr(loan_code, 13, 1) = p_product;
       
    delete from bhbfcocr.tmp_rpt_bal
     where loc_code = p_office
       and substr(loan_code, 1, 1) = P_type
       and substr(loan_code, 13, 1) = p_product;
    delete from bhbfcocr.rpt_to_date
     where loc_code = p_office
       and substr(loan_code, 1, 1) = P_type
       and substr(loan_code, 13, 1) = p_product;
  ...
  commit;
  /*           Run Beg of Month process for a Branch                 */
  bhbfcocr.lm_month_beg_process.Month_Beg_proc_Start(p_office,
                                                          '',
                                                          '',
                                                          p_product,
                                                          P_type,
  ...
  /*          Run Year End Calculation Process for a branch            */
    ...
    bhbfcocr.yr_end_calculation_process.yr_end_select_data(p_fin_year,
                                                                p_office,
                                                                p_product,
                                                                P_type,
                                                                '0000000000000',
  ...
  /*         Run Year End Classification Process for a branch          */
    ...
    bhbfcocr.yr_end_ln_classification.Ln_Class_Select_Data(p_fin_year,
                                                                p_office,
                                                                p_product,
                                                                P_type,
                                                                '0000000000000',
...
procedure Closing_half_yr (p_office in varchar2,p_fin_year in varchar2,p_period in varchar2,p_date1 in date,p_date2 in date)is
 ...
 begin
       ...
       /*          Run Year End Calculation Process for a branch            */
       for idx in(select distinct substr(m.loan_code,13,1) product, substr(m.loan_code,1,1) type  from bhbfcocr.loan_mas m where m.loc_code=p_office)loop          
               bhbfcocr.yr_end_calculation_proc_half.yr_end_select_data(p_fin_year,p_office,idx.product,idx.type,'0000000000000','99999999999999');
       end loop; 
       commit;
        /*         Run Year End Classification Process for a branch          */
       for idx in(select distinct substr(m.loan_code,13,1) product, substr(m.loan_code,1,1) type  from bhbfcocr.loan_mas m where m.loc_code=p_office)loop          
               bhbfcocr.half_yr_classification.Ln_Class_Select_Data(p_fin_year,p_office,idx.product,idx.type,'0000000000000','99999999999999');
               DBMS_OUTPUT.ENABLE (buffer_size => NULL);
       end loop; 
       commit;   
